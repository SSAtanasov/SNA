# Урок 1: Преглед на заснет мрежов трафик във Wireshark
...
### Част 2: Заснемане и анализ на локални ARP данни във Wireshark

- Стартиране и спиране на заснемането на данни от ping трафик до отдалечени хостове
- Намиране на IPv4 и MAC адресна информация в заснетите PDU пакети
- Анализиране на съдържанието на ARP съобщенията, обменени между устройствата в LAN

### Част 3: Преглед на ARP кеш записите на компютъра
...
**Address Resolution Protocol (ARP)** се използва от TCP/IP за съпоставяне на Layer 3 IPv4 адрес с Layer 2 MAC адрес. Когато Ethernet рамка се изпраща в мрежата, тя трябва да има destination MAC адрес. За да открие динамично MAC адреса на известна дестинация, source устройството излъчва (broadcast) ARP request в локалната мрежа. Устройството, което притежава IP адреса, отговаря с ARP reply съобщение, което съдържа неговия MAC адрес.
...
В Част 2 на това упражнение, ще направите ping на друг компютър в LAN и ще заснемете ARP requests и replies във Wireshark. Ще разгледате и вътре в заснетите рамки за специфична информация. Този анализ трябва да помогне за изяснение как пакетните headers се използват за транспорт на данни до тяхната дестинация.
...
### Стъпка 2: Стартиране на Wireshark и започване на заснемането на данни
...
**b.** След като Wireshark стартира, изберете мрежовия интерфейс, който идентифицирахте с командата `ipconfig`. Въведете **arp** в filter полето. Това настройва Wireshark да показва само пакети, които са част от ARP обмена между устройствата в локалната мрежа. След като сте избрали правилния интерфейс и въвели филтър информацията, кликнете **Start capturing packets** (shark fin icon), за да започнете заснемането на данни.
...
**c.** В Command Prompt прозорец, направете ping на default gateway, за да тествате свързаността към адреса на gateway, който беше идентифициран в Част 2, Стъпка 1.
...
**e.** Спрете заснемането на данни, като кликнете **Stop Capture** (червен квадрат icon) на toolbar.

### Стъпка 3: Изследване на заснетите данни
...
1. **Горната секция** показва списъка на заснетите PDU frames с резюме на IPv4 пакетната информация.
2. **Средната секция** показва PDU информацията за избраната рамка в горната част на екрана и разделя заснетата PDU рамка по нейните protocol layers.
...
## Част 3: Изследване на ARP кеш записите на компютъра
...
**b.** Във Wireshark, кликнете **Start capturing packets** (shark fin icon), за да започнете ново заснемане.

**c.** От Command Prompt, направете ping на default gateway или на друг компютър в мрежата.

**d.** Спрете заснемането във Wireshark. Ще видите нови ARP request и reply съобщения в Wireshark capture.
